...
/* The following regexp patterns are part of the python ogn-client.
 * You can find it here: https://github.com/glidernet/python-ogn-client
 * Difference: Python group (?P<ground_speed>\d{4}) becomes (?<groundSpeed>\\d{4}) --> backslashes must be escaped and no "P" and no underscores (I replaced them with camelCase)
 */
 ...
 */
	...
	String PATTERN_RECEIVER_BEACON = "" +
	    "(?:" +
	        "v(?<version>\\d+\\.\\d+\\.\\d+)" +
	        "(?:\\.(?<platform>.+?))?" +
	    "\\s)?" +
